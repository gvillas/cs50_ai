    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in actions:
            if self.get_q_value(state, action) == best_reward:
                best_action = action
        if epsilon:
            population = ["all_actions", "best_action"]
            action = random.choices(population=population, weights=[epsilon, 1-epsilon])
            if action == "all_actions":
                population = Nim.available_actions(state)
                acao = random.choice(population)
                return acao
            else:
                return best_action
        else:
